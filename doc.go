...
//
// An [Executor] is designed to be able to run Coroutines spawned in various
...
// [Coroutine] spawning is designed not to block.
...
//
// A [Coroutine] can be reactive.
//
// A Coroutine is created with an [Operation] function.
// In this user-provided function, one can return a specific [Result] to tell
// a Coroutine to watch and await some Events (any of [Signal], [State] and
// [Memo]), and the Coroutine can just re-run whenever any of these Events
// notifies.
//
...
// the Coroutine from within the Operation function. Simple.
//
// # Use Case #3: Easy State Machines
//
// A [Coroutine] can also switch from one [Operation] to another, just like
// a state machine can transit from one state to another.
...
// ends it.
//
// # Spawning Async Coroutines vs. Passing Data over Go Channels
//
